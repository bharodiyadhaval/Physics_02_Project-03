...
//Camera Params
glm::vec3 g_cameraXYZ = glm::vec3(0.0f, 0.0f, 10.0f);
...
//Create The object using the details passed to the method
...
//Load the object details from the plyFile
...
//Load SceneDetails from the Scene.txt file
...
int main(void)
{
	...
	myInterface* io = new myPhysics();

	//Init Physics Object Vector
	std::vector<myInterface::PhysicsObject> PhysicsToGraphics = io->InitializePhysics("Scene.txt");
	...
	//Load Scene.txt to the graphics
	...
	for (int i = 0; i <	gridy; i++)
	{
		...
		{
			cGameObject* Gnodeload = new cGameObject();
			Gnodeload->position = glm::vec3(9 - 3*i, 15 - 3*j, 4);
			Gnodeload->diffuseColour = glm::vec4(1.0f, 1.0f, 1.0f, 1.0f);
			Gnodeload->orientation2 = glm::vec3(0, 0, 0);
			Gnodeload->meshName = "SphereRadius1";
			Gnodeload->typeOfObject = SPHERE;
			Gnodeload->scale = 0.6f;
			Gnodeload->bIsUpdatedInPhysics = 1;

			myInterface::PhysicsObject *Pnodeload = new myInterface::PhysicsObject();
			Pnodeload->pos = Gnodeload->position;
			Pnodeload->color = Gnodeload->diffuseColour;
			Pnodeload->type = myInterface::SPHERE;
			Pnodeload->scale = Gnodeload->scale;
			Pnodeload->pos = Gnodeload->position;
			Pnodeload->name = "softbody";
			Pnodeload->or2 = Gnodeload->orientation2;
			Pnodeload->phys = 1;

			PhysicsToGraphics.push_back(*Pnodeload);
			::g_vecGameObjects.push_back(Gnodeload);
		}
	...
	cGameObject* Gnodeload = new cGameObject();
	Gnodeload->position = glm::vec3(4,11,4);
	Gnodeload->diffuseColour = glm::vec4(1.0f, 0.0f, 1.0f, 1.0f);
	Gnodeload->orientation2 = glm::vec3(0, 0, 0);
	Gnodeload->meshName = "cloth";
	Gnodeload->typeOfObject = SPHERE;
	Gnodeload->scale = 0.5;
	Gnodeload->bIsUpdatedInPhysics = 0;
	...
	//Initialize the vectors and fill them with 0s
	for (int i = 0; i < PhysicsToGraphics.size(); i++)
	{
		Displacement.push_back(glm::vec3(0, 0, 0));
		Rotation.push_back(glm::vec3(0, 0, 0));
	}

	for (int i = 0; i < PhysicsToGraphics.size(); i++)
	{
		std::cout << i << PhysicsToGraphics[i].name << "     " << PhysicsToGraphics[i].pos.x << std::endl;
	}
...
//		exit(
	...
	LoadPly("cloth", "Flat_XY_Plane_xyz.ply");
	...
	cMesh testMesh;
	testMesh.name = "cloth";
	if (!LoadPlyFileIntoMesh("Flat_XY_Plane_xyz.ply", testMesh))
	{
		std::cout << "Didn't load model" << std::endl;
		// do something??
	}
	...
	//Stores Velocity For Nodes In the Vector
	struct nodeVel
	{
		float x = 0, y = 0, z = 0;
	};

	std::vector<nodeVel> clothvec;

	//INitializing the vector with zeros for the initial Run
	for (int i = 0; i < testMesh.numberOfVertices; i++)
	{
		nodeVel Initnodes;
		Initnodes.x = 0;
		Initnodes.y = 0;
		Initnodes.z = 0;
		clothvec.push_back(Initnodes);
	}
...
//			}// if ( ::g_pVAOManager->lookupVAOFromName(
		...
		//Main Physics Method
		io->PhysicsStep((float)deltaTime, wforce, sforce, leftforce, rightforce, upforce, Displacement, Rotation, PhysicsToGraphics, collision);
		
		
		for (int i = 0; i < ::g_vecGameObjects.size(); i++)
		{
			
			::g_vecGameObjects[i]->position += Displacement[i];
			::g_vecGameObjects[i]->orientation2 += glm::radians(150.0f*Rotation[i]);
			::g_vecGameObjects[i]->diffuseColour = PhysicsToGraphics[i].color;
		}
		std::cout << g_vecGameObjects[1]->position.x << "    " << g_vecGameObjects[1]->position.y <<"      " << g_vecGameObjects[1]->position.z << std::endl;
		
		//Gravity for the softbody mesh
		glm::vec3 clothgravity = glm::vec3(0.0, -0.02, 0);
		
		//Define How Far the nodes can be before they get pulled together or pushed apart
		float nodedistance = 3.9f;
		for (int i = 0; i < testMesh.numberOfTriangles; i++)
		{
			int one, two, three;
			float a =2.0;
			//
			one = testMesh.pTriangles[i].vertex_ID_0;
			two = testMesh.pTriangles[i].vertex_ID_1;
			three = testMesh.pTriangles[i].vertex_ID_2;
		
			//If they get too close
			if (sqrt(pow((testMesh.pVertices[one].x - testMesh.pVertices[two].x), 2) + pow((testMesh.pVertices[one].y - testMesh.pVertices[two].y), 2) + pow((testMesh.pVertices[one].z - testMesh.pVertices[two].z), 2))  < nodedistance-0.1)
			{
				clothvec[one].x -= a * (testMesh.pVertices[two].x - testMesh.pVertices[one].x); // / abs(testMesh.pVertices[two].x - testMesh.pVertices[one].x + 0.001);
				clothvec[one].y -= a * (testMesh.pVertices[two].y - testMesh.pVertices[one].y); // / abs(testMesh.pVertices[two].y - testMesh.pVertices[one].y + 0.001);
				clothvec[one].z -= a * (testMesh.pVertices[two].z - testMesh.pVertices[one].z); // / abs(testMesh.pVertices[two].z - testMesh.pVertices[one].z + 0.001);

				clothvec[two].x -= a * (testMesh.pVertices[one].x - testMesh.pVertices[two].x); // / abs(testMesh.pVertices[one].x - testMesh.pVertices[two].x + 0.001);
				clothvec[two].y -= a * (testMesh.pVertices[one].y - testMesh.pVertices[two].y); // / abs(testMesh.pVertices[one].y - testMesh.pVertices[two].y + 0.001);
				clothvec[two].z -= a * (testMesh.pVertices[one].z - testMesh.pVertices[two].z); // / abs(testMesh.pVertices[one].z - testMesh.pVertices[two].z + 0.001);
			...
			else if(    sqrt(pow((testMesh.pVertices[one].x - testMesh.pVertices[two].x), 2) + pow(    (testMesh.pVertices[one].y - testMesh.pVertices[two].y), 2   ) + pow(     (testMesh.pVertices[one].z - testMesh.pVertices[two].z), 2  )        )      > nodedistance+0.1)
			{
				clothvec[one].x += a * (testMesh.pVertices[two].x - testMesh.pVertices[one].x);// / abs(testMesh.pVertices[two].x - testMesh.pVertices[one].x + 0.001);
				clothvec[one].y += a * (testMesh.pVertices[two].y - testMesh.pVertices[one].y); // / abs(testMesh.pVertices[two].y - testMesh.pVertices[one].y + 0.001);
				clothvec[one].z += a * (testMesh.pVertices[two].z - testMesh.pVertices[one].z); // / abs(testMesh.pVertices[two].z - testMesh.pVertices[one].z + 0.001);
			...
			{
				clothvec[one].x -= clothvec[one].x/600;
				clothvec[one].y -= clothvec[one].y / 600;
				clothvec[one].z -= clothvec[one].z/ 600;

				clothvec[two].x -= clothvec[two].x / 600;
				clothvec[two].y -= clothvec[two].y / 600;
				clothvec[two].z -= clothvec[two].z / 600;

				clothvec[three].x -= clothvec[three].x / 600;
				clothvec[three].y -= clothvec[three].y / 600;
				clothvec[three].z -= clothvec[three].z / 600;
			}
			...
			//Keep the gravity
			clothvec[one].y += clothgravity.y;
			clothvec[two].y += clothgravity.y;
			clothvec[three].y += clothgravity.y;

			clothvec[one].z += clothgravity.z;
			clothvec[two].z += clothgravity.z;
			clothvec[three].z += clothgravity.z;


			//Hold the cloth from the top rim
			if (one<90 && two<90 && three <90)
			{
				//testMesh.pVertices[one].x += clothvec[one].x * deltaTime / 10;
				//testMesh.pVertices[two].x += clothvec[two].x * deltaTime / 10;

				testMesh.pVertices[one].y += clothvec[one].y * deltaTime /80;
				testMesh.pVertices[two].y += clothvec[two].y * deltaTime / 80;
				testMesh.pVertices[three].y += clothvec[three].y * deltaTime / 80;

				testMesh.pVertices[one].x += clothvec[one].x * deltaTime / 80;
				testMesh.pVertices[two].x += clothvec[two].x * deltaTime / 80;
				testMesh.pVertices[three].x += clothvec[three].x * deltaTime / 80;

				testMesh.pVertices[one].z += clothvec[one].z * deltaTime / 80;
				testMesh.pVertices[two].z += clothvec[two].z * deltaTime / 80;
				testMesh.pVertices[three].z += clothvec[three].z * deltaTime / 80;




				//testMesh.pVertices[one].z += clothvec[one].z * deltaTime / 10;
				//testMesh.pVertices[two].z += clothvec[two].z * deltaTime / 10;
			}
